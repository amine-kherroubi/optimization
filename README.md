...
python3 -m bin_packing.falkenauer_benchmark --no-graphs
```
...
Results are printed as a table with columns: `Instance`, `Items`, `Capacity`, `LB` (theoretical lower bound), `Bins`, `Gap` (bins − LB), `Time (s)`, and `Method`. A summary line reports total time, average bins, and the number of provably optimal solutions (gap = 0).
