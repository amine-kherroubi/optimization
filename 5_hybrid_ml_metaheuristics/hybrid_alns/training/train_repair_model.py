...
class ModelEvaluator:
    ...
    def print_report(self, metrics: dict[str, Any]) -> None:
        ...
        print(f"  Specificity   : {metrics['specificity']:.4f}  (true negatives / actual negatives)")
        ...
        print("  High precision means fewer placement mistakes and more reliable positive predictions.")
        print("  High recall means the model finds most of the best bins and misses fewer good choices.")
        print("  ROC-AUC measures ranking quality, which is the key objective for ALNS repair.")
        print("=" * 70 + "\n")
...
def plot_feature_importance(
    ...
    if not hasattr(model, "feature_importances_"):
        print("⚠️  The model does not expose feature_importances_")
        return
...
def main() -> None:
    ...
    if args.augment_with:
        ...
        if aug_path.exists():
            ...
            print(
                f"✅ Augmented with {len(X_aug)} ALNS states -> total: {len(X)} rows"
            )

    print(
        f"✅ Dataset generated: {summary.rows} rows x {summary.cols} features"
    )
    print(f"   Positive rate: {summary.positive_rate:.4f} (expected min: {1.0/(1+args.max_negatives):.4f})")
    ...
    if args.grid_search:
        print("⚠️  Grid search enabled (very long, 1-2 hours)...")
        param_grid = {
    ...
    if metrics["roc_auc"] < 0.70:
        print("⚠️  WARNING: ROC-AUC < 0.70. Consider increasing the number of instances or features.")
