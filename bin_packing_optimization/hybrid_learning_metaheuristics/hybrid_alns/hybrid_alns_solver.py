...
class AnnealedLinUCBBandit:
    """LinUCB with α annealed according to the SA temperature ratio.

    α(t) = α_min + (α_max − α_min) × (T/T₀)

    context[0] = T/T₀  (Feature 0 from _build_context).

    ┌─────────────────┬──────────────────────────────────────────────┐
    │ Start of search │  T/T₀ ≈ 1  →  α ≈ α_max  →  explore freely  │
    │ End of search   │  T/T₀ ≈ 0  →  α ≈ α_min  →  exploit hard    │
    └─────────────────┴──────────────────────────────────────────────┘

    Mirrors the SA cooling schedule: the bandit explores when the
    algorithm is still warm and converges to exploitation as it cools —
    exactly when precision matters most.  Same O(d²) update as LinUCB.
    """

    __slots__ = ("_alpha_min", "_alpha_max", "_A_inv", "_b", "_n_arms")

    def __init__(self, n_arms: int, n_features: int, alpha: float = 0.3,
                 alpha_min: float = 0.05, alpha_max: float = 1.0):
        # `alpha` param kept for API compatibility with monkey-patching
    ...
    def _alpha(self, context: np.ndarray) -> float:
        """Compute current α from temperature ratio (context[0] = T/T₀)."""
        t_ratio = float(np.clip(context[0], 0.0, 1.0))
        return self._alpha_min + (self._alpha_max - self._alpha_min) * t_ratio
    ...
    def select_arm(self, context: np.ndarray) -> int:
        ...
        for k in range(self._n_arms):
            A_inv   = self._A_inv[k]
            theta   = A_inv @ self._b[k]
            scores[k] = theta @ context + alpha * np.sqrt(np.clip(context @ A_inv @ context, 0, None))
        return int(np.argmax(scores))

    def update(self, arm: int, context: np.ndarray, reward: float) -> None:
        """Sherman-Morrison rank-1 update — identical to LinUCBBandit."""
        A_inv = self._A_inv[arm]
        Ax    = A_inv @ context
...
class NeuralLinUCBBandit:
    """LinUCB bandit with a 2-layer ReLU MLP encoder (pure NumPy, online SGD).

    Architecture
    ------------
    context (n_features D)
        → Linear(n_features → 32) → ReLU          (hidden layer)
        → Linear(32 → 16)         → ReLU          (embedding φ)
        → disjoint LinUCB per arm on φ

    UCB score  : θ_k^T φ(x) + α √(φ(x)^T A_k^{-1} φ(x))
    UCB update : Sherman-Morrison rank-1 on A_k^{-1}  (O(d²))
    MLP update : SGD on L = (r − θ_k^T φ(x))²        (backprop, O(d·h))

    The MLP learns to extract features that maximise reward predictability,
    while LinUCB heads handle exploration/exploitation on the embedding.
    Both are updated *online* — no pre-training required.
    """
    ...
    def __init__(self, n_arms: int, n_features: int,
                 alpha: float = 0.3, lr: float = 0.005):
        self.alpha  = float(alpha)
        self._n_arms = n_arms
        self._lr    = float(lr)

        # Xavier initialisation for MLP weights
        rng = np.random.default_rng(0)
        h, e = self._N_HIDDEN, self._N_EMBED
        self._W1 = rng.normal(0, np.sqrt(2.0 / n_features), (h, n_features))
        self._b1 = np.zeros(h)
        self._W2 = rng.normal(0, np.sqrt(2.0 / h),          (e, h))
        self._b2 = np.zeros(e)

        # Disjoint LinUCB heads — one identity matrix per arm
        self._A_inv: list[np.ndarray] = [np.eye(e) for _ in range(n_arms)]
        self._b_vec: list[np.ndarray] = [np.zeros(e) for _ in range(n_arms)]

    # ── forward pass ──────────────────────────────────────────────────────────
    def _forward(self, x: np.ndarray):
        """Return (φ, h1, z1, z2) — intermediates needed for backprop."""
        z1  = self._W1 @ x + self._b1    # (H,)
        h1  = np.maximum(0.0, z1)        # ReLU
        z2  = self._W2 @ h1 + self._b2   # (E,)
        phi = np.maximum(0.0, z2)        # ReLU  →  φ(x)
        return phi, h1, z1, z2

    # ── bandit interface ───────────────────────────────────────────────────────
    def select_arm(self, context: np.ndarray) -> int:
        phi, _, _, _ = self._forward(context)
        scores = np.empty(self._n_arms)
        for k in range(self._n_arms):
            A_inv = self._A_inv[k]
            theta = A_inv @ self._b_vec[k]
            ucb   = np.clip(phi @ A_inv @ phi, 0.0, None)
            scores[k] = theta @ phi + self.alpha * np.sqrt(ucb)
        return int(np.argmax(scores))

    def update(self, arm: int, context: np.ndarray, reward: float) -> None:
        phi, h1, z1, z2 = self._forward(context)

        # ── LinUCB head: Sherman-Morrison ──────────────────────────────────────
        A_inv = self._A_inv[arm]
        Ax    = A_inv @ phi
        denom = 1.0 + phi @ Ax
        self._A_inv[arm] = A_inv - np.outer(Ax, Ax) / denom
        self._b_vec[arm] += reward * phi

        # ── MLP: SGD on L = (r − θ_k^T φ)²  (minimise prediction error) ──────
        theta = self._A_inv[arm] @ self._b_vec[arm]
        err   = reward - float(theta @ phi)     # scalar

        # d L / d φ  = −2·err·θ
        d_phi = -2.0 * err * theta              # (E,)

        # Layer 2 backward
        d_z2 = d_phi * (z2 > 0.0)              # ReLU gate
        dW2  = np.outer(d_z2, h1)
        db2  = d_z2

        # Layer 1 backward
        d_h1 = self._W2.T @ d_z2
        d_z1 = d_h1 * (z1 > 0.0)              # ReLU gate
        dW1  = np.outer(d_z1, context)
        db1  = d_z1
