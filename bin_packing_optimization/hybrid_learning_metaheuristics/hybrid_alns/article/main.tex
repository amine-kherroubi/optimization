...
by behavioral cloning of BFD, but the training data are generated as ranking
decisions rather than independent rows. In each BFD replay, labels are collected
from the current partial packing before the item is inserted: the feasible bin
with minimum post-placement slack receives label $1$, sampled alternative bins
receive label $0$, and all candidates from the same placement decision share a
group identifier. The same grouped format is used for DAgger-lite ALNS-state
collection, where random-item, worst-load, and related-item destroy moves expose
the learner to the mid-search states encountered at inference. Every pair is
encoded by eleven features normalized by capacity or instance size---item size and
its square, size rank, fraction of items still to reinsert, bin load, residual
capacity, post-placement slack, bin occupancy, the largest and smallest item
already in the bin, and the fill ratio. The trainer uses grouped holdout and
grouped cross-validation so candidates from one repair decision cannot appear in
both train and test sets, and it reports top-1 repair accuracy in addition to
row-wise ROC-AUC. A standard scaler is stored with the model, and a fast predictor
applies it in NumPy and reads class probabilities directly, avoiding per-call
validation overhead. Algorithm~\ref{alg:alns} summarizes the complete loop.
