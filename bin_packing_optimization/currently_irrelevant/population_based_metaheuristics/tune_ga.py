...
from bin_packing_optimization.IGNORE_THESE_FOR_NOW.population_based_metaheuristics.solver import BinPackingSolver

PROJECT_ROOT = Path(__file__).resolve().parents[1]
BENCHMARKS_ROOT = PROJECT_ROOT / "datasets"
RESULTS_DIR = (
    PROJECT_ROOT / "results" / "4_population_based_metaheuristics" / "ga_tuning"
)

GA_ROUNDS = 1

GA_GRID = {
    "population_size": [20, 40, 60, 100],
    "generations": [100, 200, 300, 400],
    "crossover_rate": [0.50, 0.70, 0.85, 0.95],
    "mutation_rate": [0.05, 0.10, 0.20, 0.40],
    "elite_count": [2, 4, 6, 8],
    "tournament_k": [2, 3, 4, 6],
}
...
PARAM_NAMES = list(GA_DEFAULTS)
TUNED_PARAM_NAMES = list(GA_GRID)
...
def evaluate_params(
    ...
    for instance in instances:
        for seed in seeds:
            rows.append(run_ga(instance, seed, params))
...
def selected_grid(parameters: list[str] | None) -> dict[str, list[Any]]:
    if parameters is None:
        return GA_GRID
    return {parameter: GA_GRID[parameter] for parameter in parameters}
    ...
    def _jobs(self) -> list[TuningJob]:
        jobs = [
            TuningJob(
                ...
                output=self._output_dir / "ga_tuning_global.csv",
            )
        ]
        ...
        for dataset in self._options.datasets:
            jobs.append(
                TuningJob(
                    ...
                    / f"ga_tuning_{dataset.replace('-', '_')}.csv",
                )
            )
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description=(
            "Tune GA parameters. By default, runs a full suite: global tuning "
    ...
    parser.add_argument(
        "--output",
        type=Path,
        default=RESULTS_DIR / "ga_tuning.csv",
    ...
    if args.single:
        GaTuner(options).run_job(TuningJob(args.label, args.datasets, args.output))
    else:
        GaTuningSuite(options, args.output_dir).run()
