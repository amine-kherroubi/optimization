...
class State(object):
    ...
    item_bins: list[
        int
    ]  # item_bins[i] = bin index for item i (only items < next_item are valid)
    loads: dict[int, int]  # Load of each bin
...
class BinPacking(object):
    ...
    def _generate_new_states(self, state: State) -> list[State]:
        ...
        for bin_index in range(state.bins_used):
            ...
            if item_size <= self._bin_capacity - current_load:
                ...
                new_item_bins: list[int] = state.item_bins.copy()
                new_item_bins[current_item] = bin_index
                new_states.append(
                    State(state.bins_used, new_item_bins, new_loads, current_item + 1)
                )
        ...
        new_item_bins = state.item_bins.copy()
        new_item_bins[current_item] = new_bins_used - 1
        new_states.append(
            State(new_bins_used, new_item_bins, new_loads, current_item + 1)
        )
    ...
    def _build_assignments(item_bins: list[int], bins_used: int) -> dict[int, set[int]]:
        """Reconstruct dict[bin -> set[items]] from the flat item_bins list."""
        ...
        for item_index, size in enumerate(self._sizes):
            ...
            for bin_index in range(bins_used):
                if loads[bin_index] + size <= self._bin_capacity:
                    ...
                    item_bins[item_index] = bin_index
                    placed = True
                    break
            if not placed:
                item_bins[item_index] = bins_used
                loads[bins_used] = size
                bins_used += 1

        return State(bins_used, item_bins, loads, self._number_of_items)
    ...
    def _branch_and_bound(self) -> None:
        ...
        counter: int = 0
        initial_state: State = State(0, [0] * self._number_of_items, {}, 0)
        ...
        self._solution = Solution(
            ...
            assignments=self._build_assignments(
                incumbent.item_bins, incumbent.bins_used
            ),
